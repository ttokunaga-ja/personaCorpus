...
fixture=$(mktemp -d "${TMPDIR:-/tmp}/p02-format-audit.XXXXXX")
trap 'rm -rf "$fixture"' EXIT HUP INT TERM

mkdir -p "$fixture/bin" "$fixture/progress/p02/full" \
  "$fixture/workspace/people/p02-site-reliability-engineer/home/documents"
...
home = root / 'workspace/people/p02-site-reliability-engineer/home/documents'
rows = [
 ('p02-m1-0001','txt_log','txt','notes.txt'),
 ('p02-m1-0002','pdf_text','pdf','search.pdf'),
 ('p02-full-0003','pdf_scan','pdf','scan.pdf'),
 ('p02-full-0004','docx','docx','brief.docx'),
 ('p02-full-0005','xlsx','xlsx','plan.xlsx'),
 ('p02-full-0006','pptx','pptx','review.pptx'),
 ('p02-full-0007','image','png','image.png'),
 ('p02-full-0008','ipynb','ipynb','analysis.ipynb'),
 ('p02-full-0009','domain_binary','pcap','trace.pcap'),
 ('p02-full-0010','domain_binary','sqlite','state.sqlite'),
 ('p02-full-0011','domain_binary','zip','bundle.zip'),
 ('p02-full-0012','code','py','code.py'),
]
def row(identifier, family, ext, name, schema, state):
 result = {'schema':schema,'kind':'artifact','persona':'p02','artifact_id':identifier,'scope_id':'p02-primary-01','scope_path':'documents','family':family,'physical_extension':ext,'path':'documents/'+name,'state':state}
 if schema == 'persona-corpus.full-addition-assignment/v1': result['canonical_extension'] = ext
 return result
m1 = [{'schema':'persona-corpus.m1-assignment/v1','kind':'header','persona':'p02','artifact_count':2}]
full = [{'schema':'persona-corpus.full-addition-assignment/v1','kind':'header','persona':'p02','artifact_count':10}]
...
(root/'progress/p02/m1.jsonl').write_text(''.join(json.dumps(x)+'\n' for x in m1))
(root/'progress/p02/full/full.jsonl').write_text(''.join(json.dumps(x)+'\n' for x in full))
...
raw = b'\x00\xff\x00\x00'  # filter 0, one opaque-red RGB pixel
(home/'image.png').write_bytes(b'\x89PNG\r\n\x1a\n' + png_chunk(b'IHDR', struct.pack('>IIBBBBB', 1, 1, 8, 2, 0, 0, 0)) + png_chunk(b'IDAT', zlib.compress(raw)) + png_chunk(b'IEND', b''))
(home/'analysis.ipynb').write_text('{"cells":[]}')
(home/'trace.pcap').write_bytes(b'\xd4\xc3\xb2\xa1' + struct.pack('<HHiiii', 2, 4, 0, 0, 65535, 1) + struct.pack('<IIII', 0, 0, 4, 4) + b'pcap')
...
mkdir "$fixture/external"
ln "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/notes.txt" "$fixture/external/notes.txt"
AUDIT="$root/bin/p02-format-audit" FIXTURE="$fixture" python3 - <<'PY'
import hashlib, importlib.machinery, importlib.util, os
from pathlib import Path
audit_path = os.environ['AUDIT']
loader = importlib.machinery.SourceFileLoader('p02_format_audit_fixture', audit_path)
spec = importlib.util.spec_from_loader(loader.name, loader)
module = importlib.util.module_from_spec(spec); loader.exec_module(module)
path = Path(os.environ['FIXTURE'])/'workspace/people/p02-site-reliability-engineer/home/documents/notes.txt'
module.verify_manifest_tree({'documents/notes.txt': {'bytes': path.stat().st_size, 'sha256': 'sha256:' + hashlib.sha256(path.read_bytes()).hexdigest()}}, {'documents/notes.txt': path}, 'fixture post-revision manifest')
PY
run() {
  PATH="$fixture/bin:$PATH" "$root/bin/p02-format-audit" --repo-root "$fixture" \
    --m1-ledger "$fixture/progress/p02/m1.jsonl" --full-ledger "$fixture/progress/p02/full/full.jsonl" \
    --home "$fixture/workspace/people/p02-site-reliability-engineer/home" --expected-count 12 \
    --expected-family-counts "$counts" --predecessor-manifest "$fixture/predecessor.jsonl" "$@"
}
reject_full() {
  if PATH="$fixture/bin:$PATH" "$root/bin/p02-format-audit" --repo-root "$fixture" \
    --m1-ledger "$fixture/progress/p02/m1.jsonl" --full-ledger "$1" \
    --home "$fixture/workspace/people/p02-site-reliability-engineer/home" --expected-count 12 \
...
# A staging/original outside home may legitimately hard-link a final artifact,
# but a second path within home is never an accepted artifact identity.
rm "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.py"
ln "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/notes.txt" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.py"
if run >/dev/null 2>&1; then
  echo 'expected duplicate in-home hard-link rejection' >&2
  exit 1
fi
rm "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.py"
printf '%s\n' 'print("ok")' > "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.py"
...
# A production-like distribution can explicitly include expected zero families
# (p02 has no scan PDFs or notebooks) without treating absent Counter keys as
# a mismatch.
FIXTURE="$fixture" python3 - <<'PY'
import json, os
from pathlib import Path
root = Path(os.environ['FIXTURE'])
home = root/'workspace/people/p02-site-reliability-engineer/home/documents'
rows = [json.loads(line) for line in (root/'progress/p02/full/full.jsonl').read_text().splitlines()]
rows = [row for row in rows if row.get('artifact_id') not in {'p02-full-0003', 'p02-full-0008'}]
rows[0]['artifact_count'] = 8
(root/'zero-full.jsonl').write_text(''.join(json.dumps(row)+'\n' for row in rows))
(root/'image.valid.png').write_bytes((home/'image.png').read_bytes())
(root/'trace.valid.pcap').write_bytes((home/'trace.pcap').read_bytes())
(home/'scan.pdf').unlink(); (home/'analysis.ipynb').unlink()
PY
zero_counts='{"txt_log":1,"pdf_text":1,"pdf_scan":0,"docx":1,"xlsx":1,"pptx":1,"image":1,"ipynb":0,"domain_binary":3,"code":1}'
run_zero() {
  PATH="$fixture/bin:$PATH" "$root/bin/p02-format-audit" --repo-root "$fixture" \
    --m1-ledger "$fixture/progress/p02/m1.jsonl" --full-ledger "$fixture/zero-full.jsonl" \
    --home "$fixture/workspace/people/p02-site-reliability-engineer/home" --expected-count 10 \
    --expected-family-counts "$zero_counts" --predecessor-manifest "$fixture/predecessor.jsonl" "$@"
}
run_zero | jq -e '.families.pdf_scan.actual == 0 and .families.ipynb.actual == 0' >/dev/null

# Full structural parsing must reject a signature-only/truncated image and a
# PCAP with a valid magic but incomplete packet framing.
printf '\211PNG\r\n\032\n' > "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/image.png"
if run_zero >/dev/null 2>&1; then
  echo 'expected truncated PNG rejection' >&2
  exit 1
fi
cp "$fixture/image.valid.png" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/image.png"
printf '\324\303\262\241' > "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/trace.pcap"
if run_zero >/dev/null 2>&1; then
  echo 'expected truncated PCAP rejection' >&2
  exit 1
fi
cp "$fixture/trace.valid.pcap" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/trace.pcap"
printf '%s\n' '%PDF-1.4 scan' > "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/scan.pdf"
printf '%s\n' '{"cells":[]}' > "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/analysis.ipynb"
...
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.py" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.pdf"
reject_full "$fixture/bad-swapped-code-pdf.jsonl"
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.pdf" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/code.py"
mkdir "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/empty"
if run >/dev/null 2>&1; then
  echo 'expected empty-directory rejection' >&2
  exit 1
fi
rmdir "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/empty"

# An OOXML ZIP must also have its declared main QName and package relationship.
cp "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/brief.docx" "$fixture/brief.valid.docx"
FIXTURE="$fixture" python3 - <<'PY'
import os, zipfile
from pathlib import Path
root = Path(os.environ['FIXTURE']); path = root/'workspace/people/p02-site-reliability-engineer/home/documents/brief.docx'
...
cp "$fixture/brief.valid.docx" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/brief.docx"
cp "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/plan.xlsx" "$fixture/plan.valid.xlsx"
FIXTURE="$fixture" python3 - <<'PY'
import os, zipfile
from pathlib import Path
root = Path(os.environ['FIXTURE']); path = root/'workspace/people/p02-site-reliability-engineer/home/documents/plan.xlsx'
with zipfile.ZipFile(path) as old: members = {name: old.read(name) for name in old.namelist()}
...
cp "$fixture/plan.valid.xlsx" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/plan.xlsx"
FIXTURE="$fixture" python3 - <<'PY'
import os, zipfile
from pathlib import Path
root = Path(os.environ['FIXTURE']); path = root/'workspace/people/p02-site-reliability-engineer/home/documents/brief.docx'
...
mkdir -p "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/revised"
FIXTURE="$fixture" python3 - <<'PY'
import hashlib, json, os
from pathlib import Path
root = Path(os.environ['FIXTURE'])
moves = [{"schema":"persona-corpus.p02-full-path-revision/v1","kind":"move","artifact_id":"p02-m1-0002","old_path":"documents/search.pdf","new_path":"documents/revised/search.pdf"}]
...
header = {"schema":"persona-corpus.p02-full-path-revision/v1","kind":"header","persona":"p02","revision":"p02-full-structure-v2","selected_count":len(moves),"preserved_m1_count":2,"assignment_sha256":sha(root/'progress/p02/full/full.jsonl'),"predecessor_manifest_sha256":sha(root/'predecessor.jsonl'),"mapping_rows_sha256":"sha256:"+hashlib.sha256(dump(moves).encode()).hexdigest()}
(root/'m1-overlay.jsonl').write_text(dump([header, *moves]))
moves[0]['artifact_id'] = 'p02-full-0004'
...
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/brief.docx" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/revised/brief.docx"
run --path-revision "$fixture/revision.jsonl" >/dev/null
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/revised/brief.docx" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/brief.docx"
...
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/brief.docx" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/revised/brief.docx"
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/review.pptx" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/review.bad"
FIXTURE="$fixture" python3 - <<'PY'
import hashlib, json, os
from pathlib import Path
root = Path(os.environ['FIXTURE'])
moves = [
 {"schema":"persona-corpus.p02-full-path-revision/v1","kind":"move","artifact_id":"p02-full-0004","old_path":"documents/brief.docx","new_path":"documents/revised/brief.docx"},
 {"schema":"persona-corpus.p02-full-path-revision/v1","kind":"move","artifact_id":"p02-full-0006","old_path":"documents/review.pptx","new_path":"documents/review.bad"},
]
def dump(rows): return ''.join(json.dumps(row, sort_keys=True, separators=(',', ':')) + '\n' for row in rows)
def sha(path): return 'sha256:'+hashlib.sha256(path.read_bytes()).hexdigest()
header = {"schema":"persona-corpus.p02-full-path-revision/v1","kind":"header","persona":"p02","revision":"p02-full-structure-v2","selected_count":len(moves),"preserved_m1_count":2,"assignment_sha256":sha(root/'progress/p02/full/full.jsonl'),"predecessor_manifest_sha256":sha(root/'predecessor.jsonl'),"mapping_rows_sha256":"sha256:"+hashlib.sha256(dump(moves).encode()).hexdigest()}
...
mv "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/review.bad" "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/review.pptx"

printf 'not a zip\n' > "$fixture/workspace/people/p02-site-reliability-engineer/home/documents/brief.docx"
if run --path-revision "$fixture/revision.jsonl" >/dev/null 2>&1; then
  echo 'expected malformed OOXML rejection' >&2
  exit 1
fi

printf '%s\n' 'p02 format audit tests: ok'

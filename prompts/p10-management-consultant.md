# p10 parent task start prompt

以下を新しいCodex Localタスクへ貼り付けてください。

```text
このpersonaCorpus Localタスクで p10（経営コンサルタント）1人分を担当してください。
作業ディレクトリは必ず
...
runbooks/personas/p10-management-consultant.md を完全に読んでください。
PARENT_TEMPLATEの各placeholderを次の値で固定し、その共通prompt全体を
このタスクの実行指示として、確認だけで停止せず制作開始まで進めてください。

PERSONA_ID=p10
ROLE=経営コンサルタント
BRIEF_PATH=runbooks/personas/p10-management-consultant.md
...
p10自身のparent/scope leaseだけです。他personaをshow、recover、releaseせず、
global `active_leases=0` をp10個別完了条件にしないでください。p10の所有pathだけを
作成・検証してください。
```

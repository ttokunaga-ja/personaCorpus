# p17 parent task start prompt

以下を新しいCodex Localタスクへ貼り付けてください。

```text
このpersonaCorpus Localタスクで p17（建設プロジェクトマネージャー）1人分を担当してください。
作業ディレクトリは必ず
...
runbooks/personas/p17-construction-project-manager.md を完全に読んでください。
PARENT_TEMPLATEの各placeholderを次の値で固定し、その共通prompt全体を
このタスクの実行指示として、確認だけで停止せず制作開始まで進めてください。

PERSONA_ID=p17
ROLE=建設プロジェクトマネージャー
BRIEF_PATH=runbooks/personas/p17-construction-project-manager.md

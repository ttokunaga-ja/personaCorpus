# p03 parent task start prompt

以下を新しいCodex Localタスクへ貼り付けてください。

```text
このpersonaCorpus Localタスクで p03（セキュリティ／GRCアナリスト）1人分を担当してください。
作業ディレクトリは必ず
...
runbooks/personas/p03-security-grc-analyst.md を完全に読んでください。
PARENT_TEMPLATEの各placeholderを次の値で固定し、その共通prompt全体を
このタスクの実行指示として、確認だけで停止せず制作開始まで進めてください。

PERSONA_ID=p03
ROLE=セキュリティ／GRCアナリスト
BRIEF_PATH=runbooks/personas/p03-security-grc-analyst.md

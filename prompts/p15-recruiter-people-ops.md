# p15 parent task start prompt

以下を新しいCodex Localタスクへ貼り付けてください。

```text
このpersonaCorpus Localタスクで p15（採用／People Ops）1人分を担当してください。
作業ディレクトリは必ず
...
runbooks/personas/p15-recruiter-people-ops.md を完全に読んでください。
PARENT_TEMPLATEの各placeholderを次の値で固定し、その共通prompt全体を
このタスクの実行指示として、確認だけで停止せず制作開始まで進めてください。

PERSONA_ID=p15
ROLE=採用／People Ops
BRIEF_PATH=runbooks/personas/p15-recruiter-people-ops.md

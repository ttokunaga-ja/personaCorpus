...
Rollback-ready means a tested reversal path, a scope-local evidence location, an accountable owner role, and an explicit stop condition. Evidence review order: DEP 09:18 -> Q-07 warning 09:42 -> rate limit 09:54 -> rollback approval 10:02 -> S2 09:47 -> reconciliation 11:06 -> concurrency 10:11 -> resolution 10:42 -> impact end 10:31 -> recovery 10:24.
Tested reversal path: revert the deployment package, validate Checkout p95 below 400 ms and Identity p95 below 350 ms in harbor-test, then promote aurora-east before borealis-west. Scope-local evidence location: the scope-local changes/deployments authorization rate-limit entry at 09:54 UTC. Accountable owner role: Identity on-call availability observer. Explicit stop condition: stop when Q-07 reaches the 24,000-job critical threshold; resume only after rollback approval is reconfirmed and Identity p95 remains below 350 ms.
Artifact p02-full-005861 / source p02-src-005861 / scope p02-primary-10. Synthetic training record; no customer data, credentials, secrets, or Kio claims.

## Purpose
This 2026 07 15 deployment change maintenance runbook note md reference applies to changes/deployments and binds to content spine IDs: org.ntc, term.rollback-ready, rel.change.260713, date.2026-07-15, term.no-kio-claim.
...
Record fingerprint: 3ba17b7ed1f6018f

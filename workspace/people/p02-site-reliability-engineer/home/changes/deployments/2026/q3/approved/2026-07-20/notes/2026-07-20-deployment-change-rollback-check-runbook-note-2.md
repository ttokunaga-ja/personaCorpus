...
Rollback-ready means a tested reversal path, a scope-local evidence location, an accountable owner role, and an explicit stop condition. Evidence review order: DEP 09:18 -> Q-07 warning 09:42 -> S2 09:47 -> concurrency 10:11 -> reconciliation 11:06 -> recovery 10:24 -> rate limit 09:54 -> impact end 10:31 -> rollback approval 10:02 -> resolution 10:42.
Tested reversal path: roll CHG-260713-042 back to the guarded consumer configuration, exercise the 12,000/24,000 Q-07 thresholds in harbor-test, then promote aurora-east followed by borealis-west. Scope-local evidence location: the scope-local changes/deployments consumer-concurrency entry at 10:11 UTC. Accountable owner role: Platform Reliability incident commander. Explicit stop condition: stop when one-hour burn reaches 14.6x; resume only after Q-07 is below 8,000 and short-window burn recovery is confirmed.
Artifact p02-full-002090 / source p02-src-002090 / scope p02-primary-10. Synthetic training record; no customer data, credentials, secrets, or Kio claims.
...
Record fingerprint: 1d54df2b2f99740f

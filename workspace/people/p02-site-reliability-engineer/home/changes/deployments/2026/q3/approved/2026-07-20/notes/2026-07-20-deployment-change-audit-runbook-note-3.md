...
Rollback-ready means a tested reversal path, a scope-local evidence location, an accountable owner role, and an explicit stop condition. Evidence review order: DEP 09:18 -> Q-07 warning 09:42 -> S2 09:47 -> resolution 10:42 -> concurrency 10:11 -> recovery 10:24 -> reconciliation 11:06 -> impact end 10:31 -> rollback approval 10:02 -> rate limit 09:54.
Tested reversal path: return to the guarded release, validate telemetry delivery within 10 minutes and the export-gap reconciliation path in harbor-test, then stage aurora-east and borealis-west. Scope-local evidence location: the scope-local changes/deployments CHG-260713-042 entry at rollback approval 10:02 UTC. Accountable owner role: Platform Reliability SRE duty manager. Explicit stop condition: stop when queue drain falls below 1,460 jobs/min; resume only after Q-07 is below 8,000 and short-window burn recovery is confirmed.
Artifact p02-full-003911 / source p02-src-003911 / scope p02-primary-10. Synthetic training record; no customer data, credentials, secrets, or Kio claims.

## Purpose
This 2026 07 20 deployment change audit runbook note md reference applies to changes/deployments and binds to content spine IDs: org.ntc, term.rollback-ready, rel.change.260713, date.2026-07-20, term.no-kio-claim.
...
Record fingerprint: 2b47d4993057ac60

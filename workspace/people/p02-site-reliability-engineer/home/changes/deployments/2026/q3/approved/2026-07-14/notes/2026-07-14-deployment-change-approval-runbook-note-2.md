...
Rollback-ready means a tested reversal path, a scope-local evidence location, an accountable owner role, and an explicit stop condition. Evidence review order: DEP 09:18 -> Q-07 warning 09:42 -> rollback approval 10:02 -> resolution 10:42 -> concurrency 10:11 -> impact end 10:31 -> reconciliation 11:06 -> S2 09:47 -> recovery 10:24 -> rate limit 09:54.
Tested reversal path: restore the queue-pressure guardrail and last-known-good consumer concurrency, verify a 1,460 jobs/min drain in harbor-test, then use a two-region canary. Scope-local evidence location: the scope-local changes/deployments resolution and enhanced-monitoring entry at 10:42 UTC. Accountable owner role: Checkout on-call rollback executor. Explicit stop condition: stop when queue drain falls below 1,460 jobs/min; resume only after the evidence chain is reconciled through 11:06 UTC and enhanced monitoring is armed for 24 hours.
Artifact p02-full-014011 / source p02-src-014011 / scope p02-primary-10. Synthetic training record; no customer data, credentials, secrets, or Kio claims.

## Purpose
This 2026 07 14 deployment change approval runbook note md reference applies to changes/deployments and binds to content spine IDs: org.ntc, deploy.dep-260713-017, rel.change.260713, date.2026-07-14, term.no-kio-claim.
...
Record fingerprint: 3101376412e1f0c0

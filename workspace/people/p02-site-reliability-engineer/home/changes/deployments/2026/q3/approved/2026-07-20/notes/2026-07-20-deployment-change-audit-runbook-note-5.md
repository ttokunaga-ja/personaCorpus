...
Rollback-ready means a tested reversal path, a scope-local evidence location, an accountable owner role, and an explicit stop condition. Evidence review order: DEP 09:18 -> Q-07 warning 09:42 -> rollback approval 10:02 -> reconciliation 11:06 -> S2 09:47 -> resolution 10:42 -> recovery 10:24 -> concurrency 10:11 -> impact end 10:31 -> rate limit 09:54.
Tested reversal path: return to the guarded release, validate telemetry delivery within 10 minutes and the export-gap reconciliation path in harbor-test, then stage aurora-east and borealis-west. Scope-local evidence location: the scope-local changes/deployments consumer-concurrency entry at 10:11 UTC. Accountable owner role: Checkout on-call rollback executor. Explicit stop condition: stop when capacity headroom falls below 31%; resume only after the evidence chain is reconciled through 11:06 UTC and enhanced monitoring is armed for 24 hours.
Artifact p02-full-014511 / source p02-src-014511 / scope p02-primary-10. Synthetic training record; no customer data, credentials, secrets, or Kio claims.

## Purpose
This 2026 07 20 deployment change audit runbook note md reference applies to changes/deployments and binds to content spine IDs: org.ntc, change.chg-260713-042, rel.change.260713, date.2026-07-20, term.no-kio-claim.
...
Record fingerprint: 0e49421fbd0a8a33

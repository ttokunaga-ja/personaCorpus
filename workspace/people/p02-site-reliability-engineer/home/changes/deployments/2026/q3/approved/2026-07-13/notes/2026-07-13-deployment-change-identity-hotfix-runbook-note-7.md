...
Rollback-ready means a tested reversal path, a scope-local evidence location, an accountable owner role, and an explicit stop condition. Evidence review order: DEP 09:18 -> Q-07 warning 09:42 -> rate limit 09:54 -> recovery 10:24 -> impact end 10:31 -> S2 09:47 -> rollback approval 10:02 -> concurrency 10:11 -> reconciliation 11:06 -> resolution 10:42.
Tested reversal path: revert DEP-260713-017 to its guarded predecessor, replay the 2,420 requests/s incident peak in harbor-test, and require Q-07 to remain below 12,000 before either production region. Scope-local evidence location: the scope-local changes/deployments DEP-260713-017 entry at deployment start 09:18 UTC. Accountable owner role: Checkout service owner. Explicit stop condition: stop when one-hour burn reaches 14.6x; resume only after rollback approval is reconfirmed and Identity p95 remains below 350 ms.
Artifact p02-full-007561 / source p02-src-007561 / scope p02-primary-10. Synthetic training record; no customer data, credentials, secrets, or Kio claims.

## Purpose
This 2026 07 13 deployment change identity hotfix runbook note md reference applies to changes/deployments and binds to content spine IDs: org.ntc, deploy.dep-260713-017, rel.change.260713, date.2026-07-13, term.no-kio-claim.
...
Record fingerprint: 6072236f03e01d55

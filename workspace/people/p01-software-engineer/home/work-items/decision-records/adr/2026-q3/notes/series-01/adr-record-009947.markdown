# ADR-197: decision expiry review interval

| Field | Value |
| --- | --- |
| Record | p01-src-009947 |
...
Adopt a decision expiry review interval. The lifecycle evidence names ADR-197, records the state as approved, and uses only role-based ownership. Product Alpha evidence remains 184 ms p95 against a 200 ms ceiling; Product Beta uses a 225 ms design budget.
